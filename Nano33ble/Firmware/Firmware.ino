...
/* rewritten for the Arduino Nano 33 BLE with the arduino LSM9DS1 libarary */
...
#include <Arduino_LSM9DS1.h> //importing the LSM9DS1 library
...
const byte vibePin = 21;  // changed from 23 to match the board pinout for the Nano 33 BLE
...
float lastBearing[3];   //define IMU values x,y,z 
float ax, ay, az;
float prevzValue = 0, prevyValue = 0; // previous value
float yaw;
float pitch;
...
void setup() {
  ...
  analogWrite(vibePin, 93750);  // Set to high frequency so switching noise is inaudible
...
void loop() {
  ...
  if (IMU.accelerationAvailable  ()) {
    ...
    yaw = az;
    pitch = ay;
  /*(imu.poll() & 0x04) {  // Only send data when we have new AHRS
    yaw = imu.Quat[0] * -1;
    pitch = imu.Quat[1];*/

    float theta = 0;

    theta = asin(norm(lastBearing[0], lastBearing[1], yaw, pitch)); //theta = prevzValue, prevyValue;    // using previous values from the LSM9DS1 library        //
    ...
    if (timeSinceLastDebugCommandChar >= commandLockout) {
      if (!digitalRead(btRtsPin)) {
        ...
        //dtostrf(Quat[2], 6, 4, &outgoingPacket[strlen(outgoingPacket)]);
        //outgoingPacket[strlen(outgoingPacket)] = ',';

        dtostrf(ax, 7, 4, &outgoingPacket[strlen(outgoingPacket)]);
        outgoingPacket[strlen(outgoingPacket)] = ',';

        dtostrf(ay, 7, 4, &outgoingPacket[strlen(outgoingPacket)]);
        outgoingPacket[strlen(outgoingPacket)] = ',';

        dtostrf(az, 7, 4, &outgoingPacket[strlen(outgoingPacket)]);
        outgoingPacket[strlen(outgoingPacket)] = ',';
